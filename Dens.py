...
try:
    T[:, :] = np.fromfile(data_dir+'temperature/temperature'+repr(i_image)+'.dat', dtype = 'float32').reshape(N_R, N_theta) * Tunit
except:
...
try:
    sigma[:, :] = np.fromfile(data_dir+'dens/dens'+repr(i_image)+'.dat', dtype = 'float32').reshape(N_R, N_theta) * aconst.M_sun / u.AU**2
except:
    print 'Could not load gasdens'+repr(i_image)+'.dat'
vtheta = np.zeros( (N_R, N_theta) )  # Initialize density array
try:
    vtheta[:, :] = np.fromfile(data_dir+'vtheta/vtheta'+repr(i_image)+'.dat', dtype = 'float32').reshape(N_R, N_theta) 
except:
